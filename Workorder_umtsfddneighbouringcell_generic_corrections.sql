...
--STEP0 : aggregation au niveau umtsfddneighbouringcell:
DROP TABLE IF EXISTS t_xml_step0_umtsfddneighbouringcell;
CREATE TABLE t_xml_step0_umtsfddneighbouringcell AS
...
xmlelement(
	...
	xmlattributes (umtsfddneighbouringcell as "id", 'modify' as "method"),
	xmlelement(
		name "attributes",
		xmlagg(
 ...
 AS xml_umtsfddneighbouringcell
	
FROM public.t_umtsfddneighbouringcell_generic_corrections
GROUP BY
rnc,
provisionedsystemrelease,
clusterid,
...
xmlelement(
	...
	xmlagg( xml_umtsfddneighbouringcell )
)
 AS xml_fddcell
	
FROM public.t_xml_step0_umtsfddneighbouringcell
GROUP BY
rnc,
provisionedsystemrelease,
clusterid,
...
xmlroot (
	xmlelement (
		...
		xmlelement(
			name "workorder", 
			xmlattributes ('correction param UMTSFddNeighbouringCell by Postgresql' as "name", 'PostGreSql' as "originator", '' as "description"),
			xmlagg( xml_rnc )
		)
	),
	VERSION '1.0',
