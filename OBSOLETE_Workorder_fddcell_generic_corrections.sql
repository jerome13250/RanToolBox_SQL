...
--STEP1A : aggregation des parametres simples sous fddcell:
...
FROM public.t_fddcell_generic_corrections
GROUP BY
rnc,
provisionedsystemrelease,
clusterid,
...
WHERE
replace(param_name,'FDDCell_','') NOT LIKE '%\_%'  --On enleve le nom fddcell car c'est la valeur par defaut, si pas de '_' on est sur un param direct sous fddcell
...
--STEP1B : aggregation des sous-objets sous fddcell: EdchResource
INSERT INTO t_xml_step1_fddcell
...
WHERE
replace(param_name,'FDDCell_','') LIKE 'EdchResource\_%'  --On enleve le nom fddcell car c'est la valeur par defaut
...
xmlroot (
	xmlelement (
		...
		xmlelement(
			name "workorder", 
			xmlattributes ('correction by Postggresql' as "name", 'PostGreSql' as "originator", '' as "description"),
			xmlagg( xml_rnc )
		)
	),
	VERSION '1.0',
