...
DROP TABLE IF EXISTS t_xmlremotefddcell_remotefddcell;
CREATE TABLE t_xmlremotefddcell_remotefddcell AS
...
xmlelement(
	name "RemoteFDDCell",
	xmlattributes (remotefddcell as "id", 'modify' as "method"),
	xmlelement(
		name "attributes",
		xmlagg(
...
FROM public.t_remotefddcell_generic_corrections
...
FROM public.t_xmlremotefddcell_remotefddcell
GROUP BY
rnc,
provisionedsystemrelease,
clusterid
...
xmlroot (
	xmlelement (
		...
		xmlelement(
			name "workorder", 
			xmlattributes ('correction remotefddcell by Postgresql' as "name", 'PostGreSql' as "originator", '' as "description"),
			xmlagg( xml_rnc )
		)
	),
	VERSION '1.0',
